...
信息承载面使用稳定实色，避免桌面/其它 App 透进来影响可读性；品牌强调色全局唯一：`#6D85C9`。该色亮度中等，实心底上的文字使用 `--accent-ink`，不使用白字。

| Token | 浅色 | 深色 | SwiftUI 映射 | 用途 |
|------|------|------|------|------|
| `--bg-canvas` | #F4F6FA | #191B21 | `Theme.Palette.bgCanvas` | 设置/历史详情画布 |
| `--bg-panel` | #FBFCFF | #22242B | `Theme.Palette.bgPanel` | 浮层底 |
| `--bg-content` | #FFFFFF | #2B2D35 | `Theme.Palette.bgContent` | 内容/卡片/分组实色面 |
| `--bg-control` | #ECF0F6 | #323641 | `Theme.Palette.bgControl` | 控件/键帽/语言胶囊 |
| `--bg-sidebar` | #F0F2F6 | #1F222A | `Theme.Palette.bgSidebar` | 侧栏/Popover chrome |
| `--bg-selection` | rgba(109,133,201,.14) | rgba(109,133,201,.18) | `Theme.Palette.bgSelection` | 当前项/hover 软底 |
| `--bg-skeleton` | #E6EBF3 | #3A3E48 | `Theme.Palette.bgSkeleton` | 加载骨架 |
...
| `--accent` | #6D85C9 | #6D85C9 | `Theme.Palette.accent` | 强调/主操作/聚焦 |
| `--accent-ink` | #080B1A | #080B1A | `Theme.Palette.accentInk` | 实心强调底上的文字/图标 |
| `--accent-soft` | rgba(109,133,201,.16) | rgba(109,133,201,.22) | accent.opacity | 引擎标签底/Pin 边 |
...
Tokens（颜色/字体/间距/圆角）
  └─ Primitives
       ├─ IconButton（borderless，hover 出现 --bg-control 圆底，26×26 命中区）
       ├─ Tag（引擎名/语言对，--accent-soft 底 + tag 字阶）
       ├─ LangPill（源→目标，中间 ⇄ 可点互换）
       ├─ StatusDot（success/warning/danger 小圆点 + caption 文案）
       └─ Skeleton（--bg-skeleton + shimmer，用于加载）
  └─ Blocks
       ├─ SourceBlock（原文 + 复制/朗读/收藏 + 检测语言标签）
       ├─ TranslationOutcomeCard（单个引擎结果 / error）
       ├─ PendingOutcomeCard（单个引擎 pending）
...
宽 **384**，高自适应（上限 ~460 后内滚）。`--bg-panel` 实色底 + `shadow-panel` + `radius-window`。无标题栏、无红绿灯（即用即走）。底部尖角箭头指向触发光标（CSS 三角形演示；SwiftUI 用自定义 Shape 或省略）。

结构（从上到下）：
1. **顶部细栏**（高 28）：左 `LangPill`（检测语言 → 目标，如 `EN ⇄ 中`），右侧 IconButton 组（Pin 常驻 / 收藏★ / 复制原文 / 朗读原文）。
2. **SourceBlock**：`--bg-content` 实色底，原文 body 字阶、`--label`；翻译中右上角小号 ProgressView。
3. **分隔**：8px 间距，无可见线（靠背景差）。
4. **ResultSlot 结果槽位区**：按设置页「结果顺序」展示每个已启用引擎。异步返回时只填充对应槽位，不因返回先后改变位置。
   - 头行：引擎名 `Tag`（如 GOOGLE，accent-soft 底）+ 右侧延迟 caption（`128ms`）+ 复制/朗读 IconButton。
   - 译文：`result` 字阶（15pt，比原文大），`--label`，可选文本。
   - 查词态额外：音标 callout（`--label-2`）+ 词性·释义逐行 callout。
   - Pending 态：同一槽位显示请求中 / 生成中 / 仍在生成，保留排序位置；不使用橙色边框，避免被误读为失败。
   - Error 态：danger 描边 + 文案；需要配置时显示「配置密钥」。
...
- IconButton hover：圆底 `--bg-control` 80ms。

### 3.2 InputBar 输入翻译面板

宽 **540**，单行起高 60，可增长至 4 行。Spotlight 式：无标题栏、`radius-window`、`shadow-panel`、`--bg-panel` 实色底。
...
宽 **248**，`--bg-content` 实色底，从状态栏图标下方箭头弹出。替换裸 NSMenu。
1. 四大动作行（图标 + 名称 + 右侧快捷键 caption）：划词翻译 ⌥D / 查词 ⌥E / 截图翻译 ⌥S / 输入翻译 ⌥A。hover 整行 `--bg-selection` 底。
2. 分隔线。
3. **最近**：标题 caption + 最多 3 条 `HistoryRow`（点击再翻译），「查看全部历史」链接。
4. 分隔线。
5. 底行：设置… ⌘, / 退出 ⌘Q（次要文字按钮）。菜单栏不展示引擎配置，避免低频设置挤占即时动作。

### 3.4 SettingsWindow 设置窗口

宽 **720** 高 **500**。标准标题栏 + 红绿灯。左 **侧栏**（宽 184，`--bg-sidebar` 实色，分区图标+名）：通用 / 翻译 / 识别 / 语音 / 密钥 / 快捷键 / 插件 / 关于。右内容画布（`--bg-canvas`，边距 20）：
- 通用：默认来源语言 Picker、默认目标语言 Picker、开机启动 Toggle、浮窗自动隐藏延时。
- 翻译：常用引擎直接展示；低频引擎折叠；结果顺序在本页独立折叠区调整。每行使用 `SettingRow`（名 + 辅助说明 + 状态点 + Toggle）。
...
用 `SettingRow`：label 左、控件右对齐，行高 38/44，分区整体用 `--bg-content` 实色卡片；行内只保留分隔线，不给每一行单独铺灰底。

### 3.5 HistoryWindow 历史 / 收藏（新增）

宽 **760** 高 **500**。标准窗口。左 **列表栏**（宽 280，`--bg-sidebar`）：顶搜索框 + 分段控件（全部 / 收藏）+ 语言对筛选；下方 `HistoryRow` 列表（原文单行截断 + 译文摘要 + 星标）。右 **详情**（`--bg-canvas`）：原文 SourceBlock + 结果阅读区（实色内容卡）+ 操作（复制/朗读/再翻译/删除/收藏）。空态：居中图标 + 「选择左侧记录查看详情」。
...
| 态 | 样式 |
|----|------|
| 加载 | 每个引擎槽位在设置顺序中显示 `Skeleton` 或请求中 / 生成中状态 |
| 空 | 居中：SF Symbol 占位图（`--label-3`）+ headline 标题 + callout 说明，左对齐说明文字 |
| 错误 | 对应引擎槽位显示 danger 描边 + 文案 + 需要配置时的设置入口；文案映射 ProviderError（鉴权/限流/网络/超时/未配置/插件） |
...
| 引擎结果到达 | 对应槽位从 pending 变为结果 | 120ms / ease-out |
| IconButton hover | 圆底淡入 | 80ms / ease |
| 行 hover（菜单/历史） | 背景 `--bg-selection` | 80ms |
...
1. **信息面不使用毛玻璃**：菜单、结果、设置分组、历史详情、OCR 结果这些承载文字的面必须是实色。毛玻璃只允许出现在系统菜单栏这类 OS chrome，不作为内容卡背景。
2. **层次靠画布/内容/描边，不堆阴影**：全局仅浮窗本体一处大投影，卡片间用 `--bg-canvas` / `--bg-content` 明度差区分。原因：避免「AI 味」多层 glow，也避免灰底像禁用态。
3. **强调色全局唯一 `#6D85C9`**：收藏星黄、状态点红绿橙属语义色不计入强调色预算；实心强调底统一使用深色 `accentInk` 保证可读性。
...
- 颜色：新建 `DesignTokens.swift`，将上表语义色封装为 `Color` 扩展；系统表面随明暗自适应，品牌强调色固定为 `#6D85C9`。
- 字体：封装 `Font` 扩展（`.result`/`.tag` 等），替换现有 `.system(size: 10/11/13/14)` 散值。
- 间距：用常量 `Spacing.s4/s8/...`，替换 `padding(12/14)` 魔法数。
- 表面：浮层/侧栏/Popover 使用 `Theme.Palette.bgPanel/bgSidebar/bgContent` 实色；文字承载面不使用 `.regularMaterial`。
- 组件：`SourceBlock` / `TranslationOutcomeCard` / `PendingOutcomeCard` 抽为独立 View，供 ResultPanel 与 HistoryWindow 复用。
